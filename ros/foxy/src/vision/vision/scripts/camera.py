...
class CameraPublisher(Node):
    def __init__(self):
        ...
        image_topic_name = '/{}/camera{}'.format(robotname,camera_name)              # Publish image to /cameraX/image
        self.publisher_ = self.create_publisher(Image, image_topic_name, 5)
        info_topic_name = '/camera{}/info'.format(camera_name)                # Publish camera info to /cameraX/info
        print(image_topic_name)
        self.publisher_info = self.create_publisher(CameraInfo, info_topic_name, 1)

        timer_period = 1/90  # seconds
        self.timer = self.create_timer(timer_period, self.timer_callback)
        config = simplejson.load(open(os.path.join(get_package_share_directory('vision'), 'config', 'camera_config.json')))
